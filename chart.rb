...
File.open('tmp/chart.r', 'w') do |file|
  ...
  colors = %w(red blue black green purple pink cyan).sort
  ...
  file.puts <<-postamble
    ...
    title(main='Throughput of HTTP hello-world on Heroku with 5 Dynos')
    axis(1)
    axis(2)
    legend(range_responses/6, range_responses, c(#{labels.inspect[1..-2]}), cex=0.8, col=c(#{colors.inspect[1..-2]}), pch=21:22)
  ...
  file.puts <<-postamble
    par(new=T)
    title(main='Latency of HTTP hello-world on Heroku with 5 Dynos')
    axis(1)
    axis(2, at=axTicks(2), c(0, 10, 100, 1000, 10000))
    legend(range_responses*0.85, log10(range_response_time)*0.3, c(#{labels.inspect[1..-2]}), cex=0.8, col=c(#{colors.inspect[1..-2]}), pch=21:22)
